...
$text_menu = $objAdmMenu->GetMenu('Виды деятельности');
$table = "work_type";

if(isset($cgi['add_row'])) {
	if ($cgi['name']) {
		$query = "insert into $table set name = '".addslashes($cgi[name])."'";
		my_exec($query);
  } else {
...
if(isset($cgi['upd_row'])) {
	if ($cgi['name']) {
		$query = "update $table set name='".addslashes($cgi[name])."' where id=".$cgi['num_str'];
		my_exec($query);
  } else {
    red("Не заполнены обязательные поля!!!");
  }
...
function add($num_str) {
  ...
  $body .= "<form method=post>\n";
	...
	$body .= "<tr valign=top><td colspan=2><b>Название:</b>&nbsp;&nbsp;<input name=name value='".stripslashes($name)."' size=69></td></tr>\n"; 
	$body .= "</table><BR>\n";
...
function show() {
  ...
  if ($cgi["btn_all"]) {
    ...
    $body .= "<td>".stripslashes($row['name'])."&nbsp;</td></tr>\n"; 
	}
	$body .= "</table>\n";
  return $body;
}
