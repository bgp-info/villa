...
$sql = "select * from exhibition_plan where id = ".$cgi[id];
$sth=my_exec($sql);
$row = mysql_fetch_array($sth);

$size = getimagesize("./picture_plan/pic_".$row[id].$row[ext]);
#print_arr($size);
...
echo "<img src='/picture_plan/pic_".$row[id].$row[ext]."' border=0>";
echo "</td></tr></table><br>";
